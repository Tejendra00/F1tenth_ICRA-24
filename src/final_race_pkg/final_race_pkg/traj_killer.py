...
class CurvePlotterNode(Node, QMainWindow):
    def __init__(self):
        Node.__init__(self, 'traj_killer')
        QMainWindow.__init__(self)
        self.publisher_inner = self.create_publisher(Float32MultiArray, 'traj_inner', 10)
        self.publisher_outer = self.create_publisher(Float32MultiArray, 'traj_outer', 10)
        self.curves = [
    ...
    def save_data(self, curve_index):
        ...
        with open(curve_file, 'w', newline='') as csvfile:
            ...
            # Aggregate input data and convert to float for spline calculation
            ...
            if len(point_data) <= self.curves[curve_index]['spline_order']:
                ...
                data_array = np.zeros((200, 4))
                for i in range(200):
                    x, y, z = new_points[0][i], new_points[1][i], new_points[2][i]
                    yaw = yaw_angles[i]
                    w = points_np[i % len(points), 3]
                ...
                if curve_index == 0:
                    ...
                    self.publisher_outer.publish(traj_msg)
